...
def home(request):
    ...
    context['upcoming_trip'] = Schedule.objects.filter(Q(status = 1) & Q(Q(bus__isDaily = True) | Q(schedule__gt = datetime.now()))).count()
...
def save_schedule(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if viaList:
                last_obj = Schedule.objects.last()
                
                for id in viaList:
                    loc_obj = Location.objects.filter(id=id).first()
                    last_obj.via.add(loc_obj)
                
                last_obj.save() 
                
            
                seat_count = last_obj.bus.seats
                
                for i in range(seat_count):
                    new_seat = Seats.objects.create(seat_number=i+1)
                    last_obj.seats.add(new_seat)
                    last_obj.save()
 
 
                
            # Tobe added BEFORE REVISED DOCUMENTATION
               
            # time = request.POST['time']
            
            # time = Schedule.objects.get['time']
            # convert into time
            # try:
            #     time_object = datetime.strptime(time, '%H:%M').time()
            #     if schedule:  # Update existing schedule
            #         schedule.time = time_object
            #         schedule.save()
            #     else:  # Create new schedule with valid time
            #         form.save()  # Assuming the form saves other fields
            # except ValueError:
            #     resp['msg'] += 'Invalid time format. Please use HH:MM.'
            
            
            # form.time = time_object
            # form.save()
            # print(time)
...
def scheduled_trips(request):
    if not request.method == 'POST':
        ...
        schedules = Schedule.objects.filter(Q(status = 1) & Q(Q(bus__isDaily = True) | Q(schedule__gt = datetime.now()))).all()
        ...
        schedules = Schedule.objects.filter(Q(status = 1) & Q(Q(bus__isDaily  = True) | Q(Q(schedule__year = year) & Q(schedule__month = month) & Q(schedule__day = day) )) & Q(Q(depart = depart) & Q(destination = destination ))).all()
        context['schedules'] = schedules
    ...
    if not request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            if booking is None:
                ...
                messages.success(request, f'Booking has been saved successfully. Your Booking Refderence Code is:<i> <b>{booking.code}</b> </i>', extra_tags = 'stay')
            else:
